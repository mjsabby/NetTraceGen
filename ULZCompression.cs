namespace NetTraceGen
{
    ...
    internal static class ULZCompression
    {
        ...
        public static unsafe int Compress(ArraySegment<byte> input, ArraySegment<byte> output, int level)
        {
            var arrayPool = ArrayPool<int>.Shared;

            var hashTable = default(int[]);
            var prev = default(int[]);

            try
            {
                hashTable = arrayPool.Rent(HashSize);
                prev = arrayPool.Rent(WindowSize);
                ...
                fixed (int* hashTablePtr = &hashTable[0])
                {
                    fixed (int* prevPtr = &prev[0])
                    {
                        ...
                        {
                            fixed (byte* outputPtr = &output.Array[output.Offset])
                            {
            ...
            finally
            {
                if (hashTable != null)
                {
                    arrayPool.Return(hashTable);
                }

                if (prev != null)
                {
                    arrayPool.Return(prev);
                }
            }
        }

        public static unsafe int Decompress(byte[] input, int inputLength, byte[] output, int outputLength)
        {
            fixed (byte* inputPtr = &input[0])
            {
                fixed (byte* outputPtr = &output[0])
                {
                    return Decompress(inputPtr, input.Length, outputPtr, output.Length);
                }
            }
        }
